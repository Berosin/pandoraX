...
interface SceneProps {
  colorA: string;
  colorB: string;
  /** Radians/second added to the sphere's idle spin. Ignored (but the
   * pointer-driven tilt still works) when `autoRotate` is off. */
  rotationSpeed: number;
  /** How fast the color wave travels across the fragment shader. */
  waveSpeed: number;
  /** Fresnel rim-light multiplier — higher reads as a brighter glow. */
  glowIntensity: number;
  /** Multiplies how far pointer position tilts/nudges the sphere. */
  mouseInfluence: number;
  /** Toggles the continuous idle spin; pointer tilt keeps working either way. */
  autoRotate: boolean;
}
...
export function Scene({
  ...
  useEffect(() => {
    uniforms.current.uGlowIntensity.value = glowIntensity;
  }, [glowIntensity]);
  ...
  useFrame((_state, delta) => {
    ...
    mesh.rotation.y += pointer.current.x * 0.01 * mouseInfluence;
    mesh.rotation.x = THREE.MathUtils.lerp(
      mesh.rotation.x,
      pointer.current.y * 0.4 * mouseInfluence,
      0.06
    );
